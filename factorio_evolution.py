...
def parse_evolution(server_output):
    ...
    """
    regex = re.compile(
        r"Evolution factor: ([\d\.]+)\. \(Time (\d+%)\) \(Pollution (\d+%)\) \(Spawner kills (\d+%)\)"
    )
...
def get_server_output():
    ...
    if process.returncode != 0:
        raise Exception(
            f"Error reading factorio evolution - error code {process.returncode}, stderr: '{process.stderr.strip()}', stdout: '{process.stdout.strip()}'")
...
if evolution == None:
    helpers.exit_with_CRITICAL(
        f"unable to parse server output '{server_output}'")

formatted_metrics = " ".join(
    [f"'{name}'={value}" for name, value in evolution.items()])
