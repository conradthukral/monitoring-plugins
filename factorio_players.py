...
def parse_players(server_output):
    ...
    Players (2):
      ...
      Player B
    """
    player_regex = re.compile(r"\s+(.*?)( \(online\))?")
    players = {}
...
def metrics(players):
    metrics = {f"player_{player}": online for player,
               online in players.items()}
...
def get_server_output():
    ...
    if process.returncode != 0:
        raise Exception(
            f"Error reading factorio users - error code {process.returncode}, stderr: '{process.stderr.strip()}', stdout: '{process.stdout.strip()}'")
...
formatted_players = ', '.join(players_online) if len(
    players_online) > 0 else 'nobody'
formatted_metrics = " ".join(
    [f"'{name}'={value}" for name, value in player_metrics.items()])

helpers.exit_with_OK(
    f"currently online: {formatted_players} | {formatted_metrics}")
